	...
	$("#register_alias_modal").on("show.bs.modal", function(e) {
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				} else if (/acct:(\d+)@nhz/.test(response.aliasURI) || /nacc:(\d+)/.test(response.aliasURI)) {
