var NRS = (function(NRS, $, undefined) {
	...
	NRS.submitForm = function($modal, $btn) {
		...
		if (NRS.downloadingBlockchain) {
			$modal.find(".error_message").html(i18n.t("js.waitbc")).show();
			NRS.unlockForm($modal, $btn);
			return;
		} else if (NRS.state.isScanning) {
			$modal.find(".error_message").html(i18n.t("js.bcrescan")).show();
			NRS.unlockForm($modal, $btn);
			return;
		}
		...
		if (data.recipient) {
			...
			if (!/^\d+$/.test(data.recipient) && !/^NHZ\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
		...
		if (!NRS.showedFormWarning) {
			if ("amountNHZ" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {
				if (new BigInteger(NRS.convertToNQT(data.amountNHZ)).compareTo(new BigInteger(NRS.settings["amount_warning"])) > 0) {
					...
					$modal.find(".error_message").html("You amount is higher than " + NRS.formatAmount(NRS.settings["amount_warning"]) + " NHZ. Are you sure you want to continue? Click the submit button again to confirm.").show();
					NRS.unlockForm($modal, $btn);
					return;
				}
			}
			...
			if ("feeNHZ" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {
				if (new BigInteger(NRS.convertToNQT(data.feeNHZ)).compareTo(new BigInteger(NRS.settings["fee_warning"])) > 0) {
					NRS.showedFormWarning = true;
					$modal.find(".error_message").html("You fee is higher than " + NRS.formatAmount(NRS.settings["fee_warning"]) + " NHZ. Are you sure you want to continue? Click the submit button again to confirm.").show();
					NRS.unlockForm($modal, $btn);
					return;
				}
			}
